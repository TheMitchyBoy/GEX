...
`gex_core.trading.filters.evaluate_entry_filters()` runs inside the rule-based advisor (`advisor.py`) and the live AI advisor path. Set `GEX_TRADER_STRICT_FILTERS=0` to bypass the entire block.
...
|----------|---------|-------|
| `GEX_TRADER_STRICT_FILTERS` | `1` | Entry master switch |
