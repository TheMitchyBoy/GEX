...
def main() -> None:
    ...
    parser.add_argument("--intraday-days", type=int, default=int(os.environ.get("GEX_INTRADAY_BACKFILL_DAYS", "10")))
    parser.add_argument("--daily-days", type=int, default=int(os.environ.get("GEX_DAILY_BACKFILL_DAYS", "0")))
    ...
    for ticker in tickers:
        ...
        if args.intraday_days > 0:
            results = backfill_recent_intraday(
                ticker, days=args.intraday_days, force=args.force
            )
            total = sum(results.values())
            print(f"{ticker} intraday: saved {total} minute snapshots across {len(results)} days")
